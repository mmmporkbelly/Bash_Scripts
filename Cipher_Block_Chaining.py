...
class CBC:
    def __init__(self, key, IV):
        ...
        for char in key:
            try:
                ...
                self.key.append(self.alphabet.index(char))
            except:
                raise Exception(
    ...
    def encode(self, text):
        ...
        if last_block:
            ...
            encrypted = ''
            zipped = zip(self.key[:len(last_block)], xor_block)
            for x in sorted(zipped):
                encrypted += x[1]
            result.append(encrypted)
    ...
    def decode(self, text):
        ...
        for block in blocks:
            decrypted_block = ''
            # Decrypt
            decrypt_key = zip(sorted(self.key), block)
            decrypt_key = list(decrypt_key)
            for x in self.key:
                for a, b in decrypt_key:
                    if x == a:
                        decrypted_block += b
                        decrypt_key.remove((a, b))
                        break
            # Now XOR it
            if first:
                de_xor = ''
        ...
        if last_block:
            # Decrypt it
            decrypted_block = ''
            decrypt_key = zip(sorted(self.key[:len(last_block)]), last_block)
            decrypt_key = list(decrypt_key)
            for x in self.key[:len(last_block)]:
                for a, b in decrypt_key:
                    if x == a:
                        decrypted_block += b
                        decrypt_key.remove((a, b))
...
print(test4.decode('+evaW8AHaM3yWvWuZaR'))
print(test4.encode('anoTherTESTT'))
print(test4.decode('4u4La65+NLDC'))